...
num_test=161
num_valid=20
path1='../csvprocess/R2CIE/Rtest.csv'
path2='../csvprocess/R2CIE/RtestY.csv'


# 读取训练数据和标签数据
train_data = pd.read_csv(path1,nrows=num_test)#path1 or path5
label_data = pd.read_csv(path2,nrows=num_test)
...
# 读取valid数据和标签数据
valid_data = pd.read_csv(path1).tail(num_valid)
# 从第二个路径读取整个文件，并获取后 num_valid 行数据
valid_label = pd.read_csv(path2).tail(num_valid)
